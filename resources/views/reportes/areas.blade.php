...
<table id="example" class="table table-striped display responsive nowrap" style="font-size:14px">
    ...
    <tbody>
        ...
        <tr>
            <td>{{$areas->nombre_sucursal}}</td>
            <td>{{$areas->nombre}}</td>
            <td>{{"$".number_format($areas->Total, 2)}}</td>
            <td>

                <a href="{{ route('pdf_areas_monalisa', ['id_area' => $areas->id_area]) }}" style="padding: 3px 6px;" class="btn btn-danger"><i class="far fa-file-pdf"></i></a>
                &nbsp;
